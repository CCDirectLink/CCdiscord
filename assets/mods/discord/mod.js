function onModsLoaded() {
	...
	function setActivity(){
		...
		var chapterNumber = cc.ig.TextCommand.parse("\\v[chapter.current]");
		var chapterName = cc.ig.TextCommand.parse("\\v[chapter.name."+chapterNumber+"]");
		var chapter = "";
		if(chapterNumber === "0") {
			chapter = 'Prologue - ';
		} else {
		...
		chapter += chapterName;
		var partySize = parseInt(cc.ig.TextCommand.parse("\\v[party.size]"));
		var partyMax = 3;
		if(partySize === 1) {
			partySize = undefined;
			partyMax = undefined;
		...
		var state= "Exploring";
		if(cc.ig.TextCommand.parse("\\v[pvp.active]")=== "true") 
			state = "In a PvP";
		else if(cc.ig.TextCommand.parse("\\v[combat.active]")=== "true")
			state = "In combat ";
		 
		area= (simplify.getActiveMapName()||"???").split('.')[0];
		var elementIndex = parseInt(cc.ig.TextCommand.parse("\\v[player.element]"))
